...
public class FilterAmount {
	...
	public double filter(String text) {
		...
		for (int index = 0; index < text.length(); index++) {
			...
			if (!isActiveCollect) { 
				if (this.isNumber(character)) isActiveCollect = true; // if current character is number, open for collect character
				...
				if (this.isDot(character)) { // if current character is dot
					if ((index + 1) < text.length() - 1) { // check next index is not last index
						...
						if (this.isNumber(nextCharacter)) { // if next character is number, open for collect (data will like .24, .78 this is a amount like 0.24, 0.78)
					...
					if (index > 0) { // if this character is dot and not index start
						...
						/* 
						 	Why thai character not collect ?
						 		Because it may be a number for something else, such as a document number.
						 		Example ส.12939
						*/
			...
			// check current charater is a english charater, because it have condition for spaial decrease point
			if (this.isEngCharacter(Character.toLowerCase(character))) {
				if (Character.toLowerCase(character) == 'i' && (index + 1) < text.length()) { // if character equal 'i' and next index is not last length
					char nextCheracter = text.charAt(index + 1); // get next character from next index [][i][x]
					
					// if next character is equal 'd', next amount will be get spacial decrease point
					/*
			...
			if (this.isWhiteSpace(character) || this.isThaiCharacter(character) || index == text.length() - 1) {
				...
				if (index == text.length() - 1 && this.isNumber(character)) strBuilder.append(character);
				...
				for (int builderIndex = 0; builderIndex < strBuilder.length(); builderIndex++) {
					...
					if (!this.isNumber(builderCharacter) && !this.isDot(builderCharacter) && !this.isComma(builderCharacter)) tempPoint -= this.decreasePoint;
					
					// if number start with 0 and length of builder equal 10, It might be a phone number, decrease point
					if (builderIndex == 0 && builderCharacter == '0' && strBuilder.length() == 10) tempPoint -= this.decreasePoint;
			...
			if (isActiveCollect) strBuilder.append(character);
		}
	...
	private String formatTextAmount(String text) {
		...
		for (int index = 0; index < text.length(); index++) {
			...
			if (this.isDot(c)) {
				if (isDuplicatedDot) continue; // duplicated
